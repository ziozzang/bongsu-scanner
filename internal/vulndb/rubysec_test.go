...
func TestRubysecUpdateLookupConditional(t *testing.T) {
	...
	if !slices.Contains(DefaultSources, SourceRubysec) {
		t.Fatal("rubysec must be a default source")
	}
		...
		func() {
			...
			for _, gem := range []string{"resolv", "branch", "unknown"} {
				...
				case "branch":
					...
					rubysecAssertAffected(t, r.Affected[0], map[string]bool{"3.2.0": false, "3.2.1": true, "3.2.3": true, "3.2.4": false, "3.3.0": true})
				case "unknown":
					a := r.Affected[0]
					if r.ID != "RUBYSEC-unknown-local" || len(a.Ranges) != 1 || len(a.Versions) != 0 || !reflect.DeepEqual(a.Database["rubysec_unmapped"], []any{"unsupported 1.2"}) {
...
func rubysecAssertAffected(t *testing.T, a Affected, cases map[string]bool) {
	...
	for installed, want := range cases {
		...
		for _, rg := range a.Ranges {
			if rg.Type != "ECOSYSTEM" || len(rg.Events) < 1 || len(rg.Events) > 2 {
			...
			got = got || (rg.Events[0].Introduced == "0" || lo >= 0) && beforeEnd
		}
		if got != want {
...
func TestRubysecRanges(t *testing.T) {
	for _, tc := range []struct {
		...
		{"branch only", []string{"~> 3.2.4"}, nil, map[string]bool{"3.1.9": true, "3.2.0": true, "3.2.3": true, "3.2.4": false, "3.3.0": true}},
	...
	for _, req := range []string{"unsupported 1.2", "~> invalid", ">= 1..2"} {
		ranges, why := rubysecRanges([]string{">= 5.0", req}, nil)
		if len(ranges) != 1 || !reflect.DeepEqual(why, []string{req}) {
			t.Errorf("%q: %+v %q", req, ranges, why)
		}
	}
	if ranges, why := rubysecRanges([]string{">= 2.0", ""}, nil); len(ranges) != 1 || !reflect.DeepEqual(why, []string{"empty requirement"}) {
		t.Fatal("empty requirement must preserve supported entries")
	}
}

func TestRubysecYAMLSubset(t *testing.T) {
	if got, err := rubysecBlock([]string{"\t", "\t"}, ">"); got != "" || err != nil {
	...
	if err != nil || len(r.Severity) != 1 || r.Severity[0].Score != "7.5" {
		t.Fatalf("numeric CVSS must be retained: %+v, %v", r, err)
	}
